document.addEventListener("DOMContentLoaded", function () {
    ...
    scheduleButtons.forEach(button => {
        button.addEventListener('click', async () => {
            ...
            const timeResponse = await fetch(`/Tuesday_Schedule/time_until/${classId}`);
            const timeData = await timeResponse.json();

            // שליפת סטטוס השיעור והאם המשתמשת רשומה
            const statusResponse = await fetch(`/Tuesday_Schedule/class_status/${classId}`);
    ...
    function updateSpotsFromServer(sessionId) {
        fetch(`/Tuesday_Schedule/class_status/${sessionId}`)
    ...
    function sendRequest(endpoint, sessionId) {
        fetch(`/Tuesday_Schedule${endpoint}`, {
            method: "POST",
    ...
    cancelBtn.addEventListener("click", async () => {
        ...
        const response = await fetch(`/Tuesday_Schedule/cancel_status/${classId}`);
