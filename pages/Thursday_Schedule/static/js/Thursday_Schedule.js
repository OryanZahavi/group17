document.addEventListener("DOMContentLoaded", function () {
    ...
    scheduleButtons.forEach(button => {
        button.addEventListener('click', async () => {
            ...
            const timeResponse = await fetch(`/Thursday_Schedule/time_until/${classId}`);
            const timeData = await timeResponse.json();

            // שליפת סטטוס השיעור והאם המשתמשת רשומה
            const statusResponse = await fetch(`/Thursday_Schedule/class_status/${classId}`);
    ...
    function updateSpotsFromServer(sessionId) {
        fetch(`/Thursday_Schedule/class_status/${sessionId}`)
    ...
    function sendRequest(endpoint, sessionId) {
        fetch(`/Thursday_Schedule${endpoint}`, {
            method: "POST",
    ...
    cancelBtn.addEventListener("click", async () => {
        ...
        const response = await fetch(`/Thursday_Schedule/cancel_status/${classId}`);
    ...
    function handleCancelClass(classId) {
        fetch(`/Thursday_Schedule/cancel`, {
            method: "POST",
            headers: {"Content-Type": "application/json"},
