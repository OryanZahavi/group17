...
def index():
    classes = get_classes_by_day("Wednesday")
    for cls in classes:
        cls["spots_filled"] = len(cls["registered_users"])  # כמות רשומים
        print(f"📌 מספר השיעורים המועברים ל-HTML: {len(classes)}")

    return render_template('Wednesday_Schedule.html', classes=classes)


@Wednesday_Schedule.route('/Wednesday_Schedule/time_until/<class_id>', methods=['GET'])
...
@Wednesday_Schedule.route('/Wednesday_Schedule/register', methods=['POST'])
def register():
...
@Wednesday_Schedule.route('/Wednesday_Schedule/class_status/<class_id>', methods=['GET'])
...
@Wednesday_Schedule.route('/Wednesday_Schedule/waitlist', methods=['POST'])
def waitlist():
    data = request.get_json()
    class_id = data.get('class_id')
    user_email = session.get('email')
...
@Wednesday_Schedule.route('/Wednesday_Schedule/cancel_status/<class_id>', methods=['GET'])
...
@Wednesday_Schedule.route('/Wednesday_Schedule/cancel', methods=['POST'])
def cancel():
    data = request.get_json()
    class_id = data.get('class_id')
