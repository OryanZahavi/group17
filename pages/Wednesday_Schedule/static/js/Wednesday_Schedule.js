document.addEventListener("DOMContentLoaded", function () {
    ...
    scheduleButtons.forEach(button => {
        button.addEventListener('click', async () => {
            ...
            const timeResponse = await fetch(`/Wednesday_Schedule/time_until/${classId}`);
            const timeData = await timeResponse.json();

            // שליפת סטטוס השיעור והאם המשתמשת רשומה
            const statusResponse = await fetch(`/Wednesday_Schedule/class_status/${classId}`);
    ...
    function updateSpotsFromServer(sessionId) {
        fetch(`/Wednesday_Schedule/class_status/${sessionId}`)
    ...
    function sendRequest(endpoint, sessionId) {
        fetch(`/Wednesday_Schedule${endpoint}`, {
            method: "POST",
    ...
    cancelBtn.addEventListener("click", async () => {
        ...
        const response = await fetch(`/Wednesday_Schedule/cancel_status/${classId}`);
    ...
    function handleCancelClass(classId) {
        fetch(`/Wednesday_Schedule/cancel`, {
            method: "POST",
            headers: {"Content-Type": "application/json"},
