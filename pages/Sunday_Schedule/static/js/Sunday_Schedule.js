document.addEventListener("DOMContentLoaded", function () {
    ...
    function sendRequest(endpoint, sessionId) {
        fetch(`/Sunday_Schedule${endpoint}`, { // שימוש בנתיבים כפי שהם ב-Sunday_Schedule.py
            method: "POST",
...
//                         alert(data.message);
//                         if (data.status === "success") {
//                             button.querySelector(".spots").textContent = (parseInt(button.querySelector(".spots").textContent.split('/')[0]) + 1) + "/10 רשומות";
//                         }
//                     });
//             });
//         });
...
//             const classId = document.querySelector(".session.selected").dataset.classId;
//
//             fetch("/waitlist", {
//                 method: "POST",
//                 headers: {"Content-Type": "application/json"},
