document.addEventListener("DOMContentLoaded", function () {
    ...
    scheduleButtons.forEach(button => {
        button.addEventListener('click', async () => {
            ...
            const timeResponse = await fetch(`/Monday_Schedule/time_until/${classId}`);
            const timeData = await timeResponse.json();

            // שליפת סטטוס השיעור והאם המשתמשת רשומה
            const statusResponse = await fetch(`/Monday_Schedule/class_status/${classId}`);
    ...
    function updateSpotsFromServer(sessionId) {
        fetch(`/Monday_Schedule/class_status/${sessionId}`)
    ...
    function sendRequest(endpoint, sessionId) {
        fetch(`/Monday_Schedule${endpoint}`, {
            method: "POST",
    ...
    cancelBtn.addEventListener("click", async () => {
        ...
        const response = await fetch(`/Monday_Schedule/cancel_status/${classId}`);
